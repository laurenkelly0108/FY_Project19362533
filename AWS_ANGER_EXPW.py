...
folder_path = 'C://Users/Lauren/Downloads/FY Project Datasets/ExpwSubset/ANGER_subsetExpw'

# Specify the output folder paths for the CSV and JSON files
csv_output_folder = 'C://Users/Lauren/Downloads/RESULTS/ExpW/AWS/CSV'
json_output_folder = 'C://Users/Lauren/Downloads/RESULTS/ExpW/AWS/JSON'
...
num_images = len([filename for filename in os.listdir(folder_path) if filename.endswith(('.png', '.jpg', '.jpeg'))])
...
for filename in os.listdir(folder_path):
    ...
    s3.Bucket(bucket_name).upload_file(image_file, f'ExpwSubset/ANGER_subsetExpw/{filename}')

    # Detect faces and emotions in the image using AWS Rekognition
    response = client.detect_faces(Image={'S3Object': {'Bucket': bucket_name, 'Name': f'ExpwSubset/ANGER_subsetExpw/{filename}'}}, Attributes=['ALL'])
    ...
    with open(os.path.join(json_output_folder, f'AWS_{image_name}.json'), 'w') as json_file:
        json.dump(response, json_file)

    # Increment the counter and print progress
    counter += 1
    progress = counter / num_images * 100
    remaining_time = (1 - counter / num_images) * (time.time() - start_time) / counter
    print(f"{progress:.2f}% done. Estimated time remaining: {remaining_time:.2f} seconds.")

# Write the emotions and confidence levels to a CSV file
with open(os.path.join(csv_output_folder, 'ANGRY_AWS_EXPWresults.csv'), mode='w', newline='') as csv_file:

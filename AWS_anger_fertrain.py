...
folder_path = 'C://Users/Lauren/Downloads/FY Project Datasets/FERTrain/angry'

# Specify the output folder paths for the CSV and JSON files
csv_output_folder = 'C://Users/Lauren/Downloads/RESULTS/FER/train/AWS/CSV'
json_output_folder = 'C://Users/Lauren/Downloads/RESULTS/FER/train/AWS/JSON'
...
for filename in os.listdir(folder_path):
    ...
    s3.Bucket(bucket_name).upload_file(image_file, f'FERTrain/angry/{filename}')

    # Detect faces and emotions in the image using AWS Rekognition
    response = client.detect_faces(Image={'S3Object': {'Bucket': bucket_name, 'Name': f'FERTrain/angry/{filename}'}}, Attributes=['ALL'])
    ...
    with open(os.path.join(json_output_folder, f'{filename}.json'), 'w') as json_file:
    ...
    with open(os.path.join(csv_output_folder, 'ANGRY_AWS_FERtrainresults.csv'), mode='w', newline='') as csv_file:
